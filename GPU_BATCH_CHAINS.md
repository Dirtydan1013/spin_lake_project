...
```bash
sbatch main_scripts/slurm_scripts/benchmark_qaqmc_batch_cuda.sh
sbatch main_scripts/slurm_scripts/benchmark_qaqmc_batch_cuda_all_gpus.sh
```

`BATCH_SIZES`, `M`, `WARMUP`, `STEPS`, and `ENGINES` can be overridden through
environment variables.  The Python implementation is
`main_scripts/python_scripts/probe_qaqmc_batch_cuda.py`.
