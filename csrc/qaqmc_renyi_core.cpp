...
std::array<int64_t, 3> QAQMCRenyiEngine::get_operator_counts() const {
    std::array<int64_t, 3> counts{0, 0, 0};  // site_diag, bond_diag, offdiag
    for (int replica = 0; replica < 2; ++replica) {
        for (int p = 0; p < M_total_; ++p) {
